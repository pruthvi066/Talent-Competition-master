            ...
            try
            {
                ...
                if (sortbyDate == "desc")
                {
                    var returnJobs = sortedJobs.OrderByDescending(x => x.CreatedOn).Skip((activePage - 1) * limit).Take(limit)
                     ...
                     decimal totalPages = (sortedJobs.Count() / limit);
                    var roundedTotalPages = Math.Round(totalPages, MidpointRounding.AwayFromZero);
                    return Json(new { Success = true, MyJobs = returnJobs, TotalCount = sortedJobs.Count(), TotalPages = roundedTotalPages });
                }
